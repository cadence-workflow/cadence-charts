#!/bin/sh
# Shared Elasticsearch connection utilities

# Build Elasticsearch connection parameters and set CURL_OPTS and BASE_URL
build_es_connection() {
    ...
    # Determine the host that will appear in BASE_URL for netrc matching
    if [ "$TLS_ENABLED" = "true" ] && [ -n "$SSL_SERVER_NAME" ]; then
        NETRC_HOST="$SSL_SERVER_NAME"
    else
        NETRC_HOST="$ES_HOST"
    fi
...
EOF
    ...
    # Set base URL using NETRC_HOST
    BASE_URL="$PROTOCOL://$NETRC_HOST:$ES_PORT"
