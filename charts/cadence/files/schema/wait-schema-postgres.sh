...
until
  ...
  MAIN_VERSION=$($(build_psql_cmd) -d $DB_NAME -t -c "SELECT curr_version FROM schema_version WHERE db_name = '$DB_NAME';" | xargs)
  MAIN_EXIT_CODE=$?
  ...
  if [ $MAIN_EXIT_CODE -ne 0 ]; then
    ...
    if [ "$ES_ENABLED" = "false" ]; then
      ...
      VIS_VERSION=$($(build_psql_cmd) -d $DB_VISIBILITY_NAME -t -c "SELECT curr_version FROM schema_version WHERE db_name = '$DB_VISIBILITY_NAME';" | xargs)
      VIS_EXIT_CODE=$?
